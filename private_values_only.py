...
def set_auction_order(bidders_valuation_copy):
    ...
    # order it
    item_valuation_tuples.sort(key=lambda x: x[0], reverse=True)
...
def get_winner_second_price_auction(bids):
    second_highest = 1
    highest = bids[0][1]
    winner = bids[0][0]
    ...
    for i in range(1, len(bids)):
        if bids[i][1] > highest:
            ...
            highest = bids[i][1]
            winner = bids[i][0]
        elif bids[i][1] > second_highest:
            second_highest = bids[i][1]
...
# calculate winner for first price auction 
def get_winner_first_price_auction(bids):
    highest = bids[0][1]
    winner = bids[0][0]
...
def create_sheet(sheet_name, num_bidder, num_item, gamma_input=3, gamma_scale=10, uniform_left=0, uniform_right=6):
    ...
    sheet.write(1, 4, "private value: uniform distribution ~ [" + str(uniform_left) + "," + str(uniform_right) + "]")
    ...
    for i in range(num_bidder):
        for j in range(num_item):
            private_values[i].append(int(numpy.random.uniform(uniform_left, uniform_right)) + int(numpy.random.gamma(gamma_input, gamma_scale)))
    ...
    sheet.write(row_start - 1, len(bidder_items[0]) + 1, "total utility")
    ...
    budget = int((sum_total / num_bidder) * 1.1)
    ...
    for i in range(num_item):
        ...
        for j in range(num_bidder):
            if len(bidder_items[j]) < (num_item / num_bidder):
                bid_value = min(bidders_valuation_copy[j][index], budgets[j]) # bids his valuation or remaining budget
                bids.append([j, bid_value])

        # compute round
        [winner_index, price] = get_winner_second_price_auction(bids)
    ...
    # write to sheet
    for x in range(len(bidder_items[0])):
        sheet.write(row_start - 1, x + 1, "item" + str(x + 1))
    sheet.write(row_start - 1, len(bidder_items[0]) + 1, "total utility")
    ...
    for i in range(num_item):
        ...
        # compute round
        [winner_index, price] = get_winner_first_price_auction(bids)
